...
const PersonalInfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
  ...
  const handleChange = (field, value) => {
    onChange({ ...data, [field]: value }) // ✅ FIXED
  }
  ...
  return (
    <div>
      ...
      <div className='flex items-start gap-4 mt-5'>
        ...
        <label className='relative w-16 h-16 cursor-pointer'>
          {data.image ? (
            <img
              ...
              alt="user-image"
              className='absolute inset-0 w-full h-full rounded-full object-cover ring ring-slate-300 hover:opacity-80'
            />
          ) : (
            <div className='w-16 h-16 rounded-full border flex items-center justify-center text-slate-600 hover:text-slate-700'>
        ...
        {/* TEXT */}
        {!data.image && (
          <span className='text-slate-600 hover:text-slate-700 cursor-pointer mt-4'>
            Upload user image
          </span>
        )}
